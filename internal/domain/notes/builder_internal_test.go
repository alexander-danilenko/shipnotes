...
// TestNowISOLayout locks the timestamp format to the ISO-8601-with-microseconds
// shape: "2006-01-02T15:04:05.000000".
func TestNowISOLayout(t *testing.T) {
	...
	got := fixed.Format("2006-01-02T15:04:05.000000")

	want := "2024-01-15T10:30:00.123456"
	if got != want {
		t.Errorf("layout produced %q, want %q", got, want)
	}

	// And that nowISO itself parses back as that layout (sanity check).
	if _, err := time.Parse("2006-01-02T15:04:05.000000", nowISO()); err != nil {
